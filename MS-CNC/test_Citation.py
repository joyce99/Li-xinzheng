...
A_X_s_sim = csc_matrix(utils.ComputeCosSim(A_s, p=2, q=False, k=lamb))  # 1/4 * 8
A_X_t_sim = csc_matrix(utils.ComputeCosSim(A_t, p=2, q=False, k=lamb))

X_s_sim = csc_matrix(utils.ComputeCosSim(X_s, p=2, q=True, k=lamb))  # 1/4 * 8
...
config['lr_ini'] = 0.02  # initial learning rate  0.01
numRandom = 5
microAllRandom = []
macroAllRandom = []
zero_microAllRandom = []
